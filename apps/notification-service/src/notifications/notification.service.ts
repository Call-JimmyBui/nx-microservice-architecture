...
export class NotificationService implements OnModuleInit {
  ...
  async sendOrderConfirmation(userId: string, orderId: string, amount: number): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error fetching user for order confirmation (ID: ${userId}): ${error.message}`, error.stack);
      return;
    }

    if (!userResponse || !userResponse.user || !userResponse.user.email) {
      this.logger.warn(`Could not retrieve email for user ID: ${userId}. Skipping order confirmation email.`);
      return;
    }

    const recipientEmail: string = userResponse.user.email;
    const userName: string = userResponse.user.lastName || userResponse.user.firstName || 'bạn';
    ...
    try {
        ...
        this.logger.log(`Order confirmation email sent to ${recipientEmail} for Order ID: ${orderId}`);
    } catch (error) {
        this.logger.error(`Failed to send order confirmation email to ${recipientEmail} for Order ID ${orderId}: ${error.message}`, error.stack);
        throw error;
    }
  }
  ...
  async sendPaymentConfirmation(userId: string, orderId: string, amount: number, paymentId: string): Promise<void> {
    ...
    try {
      ...
      const recipientEmail = userResponse.user.email;
      const userName = userResponse.user.lastName || userResponse.user.firstName || 'bạn';
